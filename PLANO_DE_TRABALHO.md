...
- [~] Varrer PRD, RFC, FDD e cada ADR extraindo os itens rastreáveis. (RFC e os 7 ADRs cobertos — 48 linhas; PRD e FDD ainda são placeholders, faltam varrer quando forem escritos nas Fases 4/5)
- [~] Cobertura ≥80% dos itens identificáveis nos documentos. (100% dos itens identificáveis em RFC+ADRs hoje; métrica final só fecha depois que PRD e FDD existirem)
- [x] ≥70% das linhas com Fonte = `TRANSCRICAO` e timestamp válido no formato `[hh:mm] Nome`. (39/48 = ~81%)
- [x] ≥5 linhas com Fonte = `CODIGO` e caminho de arquivo real. (9 linhas, todos os caminhos conferidos com `find`)
- [x] Regra prática: se não der pra preencher "Localização" com uma origem concreta, o item provavelmente foi inventado — ajustar ou remover do documento de origem. (aplicada: itens com sourcing incerto, ex. alguns bullets de "Consequências" nos ADRs, foram deixados de fora em vez de inventar timestamp)
